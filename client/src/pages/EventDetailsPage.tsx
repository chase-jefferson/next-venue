...
const EventDetails = () => {
    ...
    const fetchEventDetails = async () => {
        try {
            if (eventId) {
                ...
                const attendeesWithRSVP = data.attendees.map((attendee: { id: string; name: string; rsvp?: "Yes" | "No" | "Maybe" }) => ({
                    ...attendee,
    ...
    const handleRSVP = async (response: "Yes" | "No" | "Maybe") => {
        try {
            ...
            const updatedAttendees = (await rsvpEvent(eventId, response)).map(function (attendee: { id: string; name: string; rsvp?: "Yes" | "No" | "Maybe" }) {
                return ({
                    ...attendee,
                    rsvp: attendee.rsvp || "Maybe"
                });
        ...
        <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
            {event ? (
                <>
                    ...
                    <p className="text-gray-700"><strong>Weather Forecast:</strong> {weather || "Loading..."}</p>

                    <h2 className="text-2xl font-semibold text-gray-800 mt-6">Attendees</h2>
                    <ul className="list-disc list-inside text-gray-700 mt-2">
                        {attendees.map((user) => (
                            <li key={user.id}>{user.name} - <span className="font-semibold">{user.rsvp}</span></li>
                        ))}
                    </ul>
